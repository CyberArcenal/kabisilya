// src/renderer/pages/farms/bukid/components/ChangeStatusModal.tsx
...
const ChangeStatusModal: React.FC<ChangeStatusModalProps> = ({
  ...
  // If no valid transitions, show a message and close automatically after a short delay
  React.useEffect(() => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Change Farm Status" size="sm">
      {availableStatuses.length === 0 ? (
        <div className="text-center py-4">
          <p className="text-sm text-[var(--text-secondary)]">
            This farm cannot change status because it is already <strong>{currentStatus}</strong>.
          </p>
        ...
        <div className="space-y-4">
          <p className="text-sm text-[var(--text-secondary)]">
            Change status for <strong>{bukidName}</strong> from{' '}
