// src/renderer/pages/farms/pitak/components/ChangePitakStatusModal.tsx
import React, { useState, useEffect } from 'react';
import Modal from '../../../../components/UI/Modal';
import Button from '../../../../components/UI/Button';

interface ChangePitakStatusModalProps {
  isOpen: boolean;
  onClose: () => void;
...
const ChangePitakStatusModal: React.FC<ChangePitakStatusModalProps> = ({
  isOpen,
  onClose,
  pitakLocation,
  currentStatus,
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Change Plot Status" size="sm">
      {availableStatuses.length === 0 ? (
        <div className="text-center py-4">
          <p className="text-sm text-[var(--text-secondary)]">
            This plot cannot change status because it is already <strong>{currentStatus}</strong>.
          </p>
        ...
        <div className="space-y-4">
          <p className="text-sm text-[var(--text-secondary)]">
            Change status for <strong>{pitakLocation}</strong> from{' '}
...
export default ChangePitakStatusModal;