...
const WorkerActionsDropdown: React.FC<WorkerActionsDropdownProps> = ({
  ...
  const isLocked = worker.status === "terminated";
  ...
  const handleDeleteClick = async () => {
    ...
    if (!confirmed) return;
    handleAction(() => onDelete(worker.id));
  };

  const handleStatusClick = () => {
    if (isLocked) {
      dialogs.warning(
        `Cannot change status of a terminated worker.`,
        "Status Locked"
      );
      setIsOpen(false);
      return;
    }
    handleAction(() => onChangeStatus(worker));
  };
    ...
    <div className="worker-actions-dropdown-container" ref={dropdownRef}>
      ...
      {isOpen && (
        ...
        >
          <div className="py-1">
            <button
              onClick={() => handleAction(() => onView(worker))}
            ...
            {!isLocked && (
              <button
                onClick={() => handleAction(() => onEdit(worker))}
                className="w-full flex items-center gap-2 px-4 py-2 text-sm transition-colors"
                style={{ color: "var(--text-primary)" }}
                onMouseEnter={(e) => {
                  e.currentTarget.style.backgroundColor = "var(--card-hover-bg)";
